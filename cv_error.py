...
def cv_error(X_train,y_train,k, method = 'OLS', alpha = 1):
    ...
    # for each partition
    for train_index, validation_index in kf.split(Xm):        # instantiate model
    ...
    # create a dataframe containing coeficients
        ...
        coefs_dataframe = pd.DataFrame([[fold[x] for x in range(X_train.shape[1])] for fold in coef_v])
    coefs_dataframe.columns = list(X_train.columns)
    return coefs_dataframe
